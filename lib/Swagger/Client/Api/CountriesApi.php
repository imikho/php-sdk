...
class CountriesApi
{
     ...
     * @param string[] $filter_region Filtration by region. (optional)
     * @param string[] $filter_sub_region Filtration by sub region. (optional)
     * @param string[] $sort Sort params:&lt;br&gt;  | ASC | DESC | |-----|------| | name | -name | | area | -area | | population | -population | | region | -region | | sub_region | -sub_region | (optional)
     * @throws \OFT\ApiException on non-2xx response
     * @return \Swagger\Client\Model\CountriesResponse
     */
    public function countriesGet($page_number = null, $page_size = null, $filter_region = null, $filter_sub_region = null, $sort = null)
    {
        list($response) = $this->countriesGetWithHttpInfo($page_number, $page_size, $filter_region, $filter_sub_region, $sort);
        return $response;
    }
     ...
     * @param string[] $sort Sort params:&lt;br&gt;  | ASC | DESC | |-----|------| | name | -name | | area | -area | | population | -population | | region | -region | | sub_region | -sub_region | (optional)
     * @throws \OFT\ApiException on non-2xx response
     * @return array of \Swagger\Client\Model\CountriesResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function countriesGetWithHttpInfo($page_number = null, $page_size = null, $filter_region = null, $filter_sub_region = null, $sort = null)
    {
        // parse inputs
        ...
        if (is_array($filter_region)) {
            $filter_region = $this->apiClient->getSerializer()->serializeCollection($filter_region, 'csv', true);
        }
        if ($filter_region !== null) {
            $queryParams['filter[region]'] = $this->apiClient->getSerializer()->toQueryValue($filter_region);
        }
        // query params
        if (is_array($filter_sub_region)) {
            $filter_sub_region = $this->apiClient->getSerializer()->serializeCollection($filter_sub_region, 'csv', true);
        }
        if ($filter_sub_region !== null) {
            $queryParams['filter[sub_region]'] = $this->apiClient->getSerializer()->toQueryValue($filter_sub_region);
        }
        // query params
        if (is_array($sort)) {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Client\Model\CountriesResponse',
                '/countries'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Client\Model\CountriesResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Client\Model\CountriesResponse', $e->getResponseHeaders());
     ...
     * @throws \OFT\ApiException on non-2xx response
     * @return \Swagger\Client\Model\CountryResponse
     */
    public function countriesIdGet($id)
    {
        list($response) = $this->countriesIdGetWithHttpInfo($id);
        return $response;
    }

    /**
     * Operation countriesIdGetWithHttpInfo
     *
     * Country by ID
     *
     * @param string $id Unique ID. (required)
     * @throws \OFT\ApiException on non-2xx response
     * @return array of \Swagger\Client\Model\CountryResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function countriesIdGetWithHttpInfo($id)
    {
        // verify the required parameter 'id' is set
        if ($id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $id when calling countriesIdGet');
        }
        // parse inputs
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Client\Model\CountryResponse',
                '/countries/{id}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Client\Model\CountryResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Client\Model\CountryResponse', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
            }
