...
/**
 * PaymentProvidersApi Class Doc Comment
 *
 * @category Class
 * @package  Swagger\Client
 * @author   Swagger Codegen team
...
class PaymentProvidersApi
{
     ...
     * @param string[] $filter_types Filtering by types. (optional)
     * @param string[] $filter_sales_channels Filtering by sales channels. (optional)
     * @param string[] $filter_features Filtering by features. (optional)
     * @param string[] $sort Sort params:&lt;br&gt;  | ASC | DESC | |-----|------| | name | -name | | alias | -alias | (optional)
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @return \Swagger\Client\Model\PaymentProvidersResponse
     */
    public function paymentProvidersGet($page_number = null, $page_size = null, $filter_name = null, $filter_alias = null, $filter_types = null, $filter_sales_channels = null, $filter_features = null, $sort = null)
    {
        list($response) = $this->paymentProvidersGetWithHttpInfo($page_number, $page_size, $filter_name, $filter_alias, $filter_types, $filter_sales_channels, $filter_features, $sort);
        return $response;
    }
     ...
     * @param string[] $sort Sort params:&lt;br&gt;  | ASC | DESC | |-----|------| | name | -name | | alias | -alias | (optional)
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @return array of \Swagger\Client\Model\PaymentProvidersResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function paymentProvidersGetWithHttpInfo($page_number = null, $page_size = null, $filter_name = null, $filter_alias = null, $filter_types = null, $filter_sales_channels = null, $filter_features = null, $sort = null)
    {
        // parse inputs
        ...
        if (is_array($filter_types)) {
            $filter_types = $this->apiClient->getSerializer()->serializeCollection($filter_types, 'csv', true);
        }
        if ($filter_types !== null) {
            $queryParams['filter[types]'] = $this->apiClient->getSerializer()->toQueryValue($filter_types);
        }
        // query params
        if (is_array($filter_sales_channels)) {
            $filter_sales_channels = $this->apiClient->getSerializer()->serializeCollection($filter_sales_channels, 'csv', true);
        }
        if ($filter_sales_channels !== null) {
            $queryParams['filter[sales_channels]'] = $this->apiClient->getSerializer()->toQueryValue($filter_sales_channels);
        }
        // query params
        if (is_array($filter_features)) {
            $filter_features = $this->apiClient->getSerializer()->serializeCollection($filter_features, 'csv', true);
        }
        if ($filter_features !== null) {
            $queryParams['filter[features]'] = $this->apiClient->getSerializer()->toQueryValue($filter_features);
        }
        // query params
        if (is_array($sort)) {
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Client\Model\PaymentProvidersResponse',
                '/payment-providers'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Client\Model\PaymentProvidersResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Client\Model\PaymentProvidersResponse', $e->getResponseHeaders());
    ...
    /**
     * Operation paymentProvidersIdGet
     *
     * Payment provider by ID
     *
     * @param string $id Unique ID. (required)
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @return \Swagger\Client\Model\PaymentProviderResponse
     */
    public function paymentProvidersIdGet($id)
    {
        list($response) = $this->paymentProvidersIdGetWithHttpInfo($id);
        return $response;
    }

    /**
     * Operation paymentProvidersIdGetWithHttpInfo
     *
     * Payment provider by ID
     *
     * @param string $id Unique ID. (required)
     * @throws \Swagger\Client\ApiException on non-2xx response
     * @return array of \Swagger\Client\Model\PaymentProviderResponse, HTTP status code, HTTP response headers (array of strings)
     */
    public function paymentProvidersIdGetWithHttpInfo($id)
    {
        // verify the required parameter 'id' is set
        if ($id === null) {
            throw new \InvalidArgumentException('Missing the required parameter $id when calling paymentProvidersIdGet');
        }
        // parse inputs
        $resourcePath = "/payment-providers/{id}";
        $httpBody = '';
        $queryParams = [];
        $headerParams = [];
        $formParams = [];
        ...
        try {
            list($response, $statusCode, $httpHeader) = $this->apiClient->callApi(
                ...
                $httpBody,
                $headerParams,
                '\Swagger\Client\Model\PaymentProviderResponse',
                '/payment-providers/{id}'
            );

            return [$this->apiClient->getSerializer()->deserialize($response, '\Swagger\Client\Model\PaymentProviderResponse', $httpHeader), $statusCode, $httpHeader];
        } catch (ApiException $e) {
            switch ($e->getCode()) {
                case 200:
                    $data = $this->apiClient->getSerializer()->deserialize($e->getResponseBody(), '\Swagger\Client\Model\PaymentProviderResponse', $e->getResponseHeaders());
                    $e->setResponseObject($data);
                    break;
            }
